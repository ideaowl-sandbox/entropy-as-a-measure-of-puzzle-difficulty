    ...
    function drawTrajectories() {
        ...
        for (let i = 0; i < g.s.puzzlesForTrajectories.length; i++) {
            ...
            if (puzzleMeta.calcPuzzleObj === undefined) {
                ...
                puzzleMeta.visualPuzzleObj = new WitnessPuzzle(puzzleMeta.setup, Object.assign({ classed: ['small'] }, { targetElement: '.' + 'puzzleTrajectoryDiv-' + i, userInterface: false }))
            }
        }
        ...
        trajectoriesDiv.each(function(d){
            ...
            currSVG.append('path')
                ...
                .attr('stroke-width', '2')
                .attr('stroke', 'blue')
                .attr('d', 'M0 ' + height + ' ' + d.calcPuzzleObj._state.puzzle.history.trajectory.map((traj, i) => "L" + (i * 10) + " " + (height - traj * 0.8)).join('') )
    ...
    function drawUniquePuzzles() {
        ...
        g.data.puzzleMetaUniqueSolutions
            ...
            .forEach((d,i)=>{
                ...
                let puzzle = new WitnessPuzzle(d.setup, Object.assign({ classed: ['small'] }, { targetElement: '.' + 'puzzleDiv-' + i, userInterface: false })); //options { render: false }    
    ...
    function drawSingleConstraintDifferencePairs() {
        ...
        let selectedSets = g.data.complexSimplerSets
            ...
            .forEach((d,i) => {
                ...
                let simplerPuzzle = new WitnessPuzzle(d.simplerPuzzle.setup, Object.assign({ classed: ['small'] }, { targetElement: '#diffPairs-' + i + '-simpler', userInterface: false })); //options { render: false }    
                let complexPuzzle = new WitnessPuzzle(d.complexPuzzle.setup, Object.assign({ classed: ['small'] }, { targetElement: '#diffPairs-' + i + '-complex', userInterface: false })); //options { render: false }    

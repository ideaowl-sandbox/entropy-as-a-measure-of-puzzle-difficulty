...
class WitnessPuzzle_Visualization {
    ...
    render() {
        ...
        if (puzzle.setup.constraints !== undefined) {
            ...
            if (puzzle.setup.constraints.cliqueCircles !== undefined && puzzle.setup.constraints.cliqueCircles.length !== 0) {
                ...
                let filledSquareCircles = svg.select('.witnessPuzzle').selectAll('.filledSquareCircle')
                    .data(puzzle.setup.constraints.cliqueCircles, d => d[0] + ' ' + d[1]);
            ...
            // // If there are colored squares, draw
            // if (puzzle.setup.constraints.brokenJunctions !== undefined && puzzle.setup.constraints.brokenJunctions.length !== 0) {

            //     let fullBrokenJunctions = puzzle.setup.constraints.brokenJunctions.filter(d => d[0] === parseInt(d[0]) && d[1] === parseInt(d[1]))
            //     let partBrokenJunctions = puzzle.setup.constraints.brokenJunctions.filter(d => d[0] !== parseInt(d[0]) || d[1] !== parseInt(d[1]))

            //     let partBrokenJunction = svg.select('.witnessPuzzle').selectAll('.partBrokenJunction')
            //         .data(partBrokenJunctions, d=> d[0] + ' ' + d[1])
            //         .enter()
            //         .append('line')
            //         .classed('partBrokenJunction', true)
            ...
            //                 expandedFullBrokenJunctions.push([d[0], d[1], newX, newY]);
            //             }
            //         }
            //     })


            //     let expandedFullBrokenJunction = svg.select('.witnessPuzzle').selectAll('.expandedFullBrokenJunctions')
            //         .data(expandedFullBrokenJunctions, d => d.join(' '))
            //         .enter()
            //         .append('line')
            //         .classed('expandedFullBrokenJunctions', true)
            //         .attr('x1', d => (d[0] + 1) * dSettings.spacing)
            //         .attr('y1', d => self._state.canvasSize - (d[1] + 1) * dSettings.spacing)
            //         .attr('x2', d => (d[2] + 1) * dSettings.spacing)
            //         .attr('y2', d => self._state.canvasSize - (d[3] + 1) * dSettings.spacing)
        ...
        if (puzzle.__generated.isEndGame) {
            ...
            if (puzzle.setup.constraints !== undefined && puzzle.setup.constraints.cliqueCircles !== undefined && puzzle.setup.constraints.cliqueCircles.length !== 0) {

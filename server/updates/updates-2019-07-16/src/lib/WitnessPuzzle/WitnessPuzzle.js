...
class WitnessPuzzle {
    ...
    setNewPuzzle(puzzleSetup, options) {
        ...
        if (self._settings.options.render) {
            ...
            let expandedFullBrokenJunctions = [];
            if (puzzle.setup.constraints && puzzle.setup.constraints.brokenJunctions) {

                let fullBrokenJunctions = puzzle.setup.constraints.brokenJunctions.filter(d => d[0] === parseInt(d[0]) && d[1] === parseInt(d[1]))

                fullBrokenJunctions.forEach(d => {
                    for (let dir of [[0, 1], [0, -1], [-1, 0], [1, 0]]) {
                        let newX = dir[0] + d[0];
                        let newY = dir[1] + d[1];
                        if (newX >= 0 && newX <= puzzle.setup.size[0] && newY >= 0 && newY <= puzzle.setup.size[1]) {
                            // expandedFullBrokenJunctions.push([newX, newY]);
                        }
                    }
                    expandedFullBrokenJunctions.push([d[0], d[1]]);
                })
            }
            expandedFullBrokenJunctions = expandedFullBrokenJunctions.map((d) => d[0] + ' ' + d[1]);
            ...
            for (let x = 0; x <= puzzle.setup.size[0]; x++) {
                ...
                let yBreaks = [];
                if (puzzle.setup.constraints && puzzle.setup.constraints.brokenJunctions) {
                    yBreaks = puzzle.setup.constraints.brokenJunctions.filter(d => d[0] === x && d[1] !== parseInt(d[1]));
                }
                for (let y = 0; y <= puzzle.setup.size[1]; y++) {
                    let isAtBrokenJunction = expandedFullBrokenJunctions.indexOf(x + ' ' + y) > -1;
                    let hasInBetweenBreak = yBreaks.filter(d => d[1] > y && d[1] < y + 1).length > 0;
                    if (!isAtBrokenJunction) {
                        if (startY === null) {
                            startY = y;
                        }
            ...
            for (let y = 0; y <= puzzle.setup.size[1]; y++) {
                ...
                let xBreaks = [];
                if (puzzle.setup.constraints && puzzle.setup.constraints.brokenJunctions) {
                    xBreaks = puzzle.setup.constraints.brokenJunctions.filter(d => d[1] === y && d[0] !== parseInt(d[0]));
                }
                for (let x = 0; x <= puzzle.setup.size[0]; x++) {
                    let isAtBrokenJunction = expandedFullBrokenJunctions.indexOf(x + ' ' + y) > -1;
    ...
    attemptSolveWithPath(moves) {
        ...
        for (let i = 0; i < moves.length; i++) {
            ...
            if (outcome && outcome.hitBrokenJunction) {
    ...
    attemptMove(userMove, opts) {
        ...
        let newPositionEntersPartlyBrokenJunction = false;
        let newPositionEntersFullyBrokenJunction = false;
        ...
        if (self._state.puzzle.setup.constraints && self._state.puzzle.setup.constraints.brokenJunctions && self._state.puzzle.setup.constraints.brokenJunctions.filter(d => JSON.stringify(d) === JSON.stringify(newPosition)).length > 0) {
            newPositionEntersFullyBrokenJunction = true;
        }
        if (self._state.puzzle.setup.constraints 
            && self._state.puzzle.setup.constraints.brokenJunctions 
            && self._state.puzzle.setup.constraints.brokenJunctions.filter(d => 
            ...
            newPositionEntersPartlyBrokenJunction = true;
        }
        ...
        if (isPuzzleReset) {
            ...
            newPositionEntersFullyBrokenJunction = false;
            newPositionEntersPartlyBrokenJunction = false;
            history.numResets += 1;
            // history.snakePositions = [JSON.parse(JSON.stringify(self._state.puzzle.setup.startPosition))];
        ...
        if (opts && opts.stopAtFailure) {
            if (newPositionEntersFullyBrokenJunction || newPositionEntersPartlyBrokenJunction) {
                return {hitBrokenJunction: true};
            }
        }        
        ...
        if (isRetractingMove) {
            ...
            if (!newPositionIsOutOfBounds && !newPositionEntersFullyBrokenJunction) {
                if (!newPositionEntersCrossedPosition && !newPositionEntersPartlyBrokenJunction) {
                    ...
                    history.lastMoveAttemptedToCrossPartlyBrokenPosition = newPositionEntersPartlyBrokenJunction;
                }
            } 
        }
    ...
    __checkIsPuzzleSolved() {
        ...
        if (puzzle.setup.constraints !== undefined) {
            ...
            if (puzzle.setup.constraints.mustCrosses !== undefined && puzzle.setup.constraints.mustCrosses.length !== 0) {
                ...
                for (let i = 0; i < puzzle.history.snakePositions.length; i++) {
                    ...
                    for (let j = mustCrosses.length - 1; j >= 0; j--) {
                        ...
                        if (!noNextPosition) {
                            xCrossed = xCrossed || (mustCrosses[j][0] > puzzle.history.snakePositions[i][0] && mustCrosses[j][0] < puzzle.history.snakePositions[i + 1][0]);
                            yCrossed = yCrossed || (mustCrosses[j][1] > puzzle.history.snakePositions[i][1] && mustCrosses[j][1] < puzzle.history.snakePositions[i + 1][1]);
                        }
            ...
            if (puzzle.setup.constraints.cliqueCircles !== undefined && puzzle.setup.constraints.cliqueCircles.length !== 0) {
                ...
                for (let [k, positionsInGroup] of Object.entries(puzzle.__generated.squareGroups)) {
                    ...
                    for (let position of positionsInGroup) {
                        let foundPosition = puzzle.setup.constraints.cliqueCircles.filter(d => d[0] === position[0] && d[1] === position[1]);

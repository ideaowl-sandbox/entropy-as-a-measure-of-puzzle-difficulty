...
setTimeout(function(){
    ...
    d3.queue()
        .defer(d3.json, './data/litw-2020-01-15.json')
        // .defer(d3.json, './data/puzzlemeta_unique_solutions.json')

    // 
        .awaitAll(function (err, [litwData]) {
            
            function detectmob(uAgent) {
            ...
            // 251
            console.log(litwData);        


            let fromLive = litwData.filter(d => Date.parse(d.questionaireDemographicsAnsweredAt) > Date.parse("2019-12-11"));

            console.log(fromLive);

            console.log('fromLive', fromLive.length)
            console.log('fromLive Mobile', (Math.round(fromLive.filter(d => detectmob(d.userAgent)).length / fromLive.length * 1000) / 10) + '%', fromLive.filter(d =>detectmob(d.userAgent)).length)

            let finishedFirstQuestionaire = fromLive.filter(d => d.questionaireDemographics !== undefined)

            console.log('finishedFirstQuestionaire', (Math.round(finishedFirstQuestionaire.length/ fromLive.length * 1000)/10) + '%', finishedFirstQuestionaire.length)
            console.log('finishedFirstQuestionaire Mobile', (Math.round(finishedFirstQuestionaire.filter(d => detectmob(d.userAgent)).length / finishedFirstQuestionaire.length * 1000) / 10) + '%', finishedFirstQuestionaire.filter(d => detectmob(d.userAgent)).length)

            let playerIsNew = finishedFirstQuestionaire.filter(d => d.questionaireDemographics.data.isTestTakenBefore === 'no');
            console.log('playerIsNew', (Math.round(playerIsNew.length / finishedFirstQuestionaire.length * 1000) / 10) + '%', playerIsNew.length)
            // console.log('finishedSimpleTrainingExampleAndNew Mobile', finishedSimpleTrainingExampleAndNew.filter(d => detectmob(d.userAgent)))

            let didNotFinishSimpleTrainingExample = playerIsNew.filter(d => d.solveAttempts.length === 0);
            console.log('didNotFinishSimpleTrainingExample', (Math.round(didNotFinishSimpleTrainingExample.length / playerIsNew.length * 1000) / 10) + '%', didNotFinishSimpleTrainingExample.length)
            console.log('didNotFinishSimpleTrainingExample Mobile', (Math.round(didNotFinishSimpleTrainingExample.filter(d => detectmob(d.userAgent)).length / didNotFinishSimpleTrainingExample.length * 1000) / 10) + '%', didNotFinishSimpleTrainingExample.filter(d => detectmob(d.userAgent)).length)

            let finishedSimpleTrainingExample = playerIsNew.filter(d => d.solveAttempts.length > 0);
            console.log('finishedSimpleTrainingExample', (Math.round(finishedSimpleTrainingExample.length / playerIsNew.length * 1000) / 10) + '%', finishedSimpleTrainingExample.length)
            console.log('finishedSimpleTrainingExample Mobile', (Math.round(finishedSimpleTrainingExample.filter(d => detectmob(d.userAgent)).length / finishedSimpleTrainingExample.length * 1000) / 10) + '%', finishedSimpleTrainingExample.filter(d => detectmob(d.userAgent)).length)


            let experimentSpreadByKeys = {};

            for (let rec of finishedSimpleTrainingExample) {
                if (experimentSpreadByKeys[rec.experiments[0].setID] === undefined) {
                    experimentSpreadByKeys[rec.experiments[0].setID] = 0;
                }
                experimentSpreadByKeys[rec.experiments[0].setID] += 1;
            }
            // console.log(finishedSimpleTrainingExample)

            

            for (let [exp, totals] of Object.entries(experimentSpreadByKeys).sort((a,b) => parseInt(a[0]) - parseInt(b[0]))) {
                console.log(exp, totals)
                let expRecs = finishedSimpleTrainingExample.filter(d => d.experiments[0].setID === exp);
                let num10 = 0;
                let num10Solved = 0;
                for (let num of [3,6,9, 10]) {
                    let numGotTo = expRecs.filter(d=>d.experiments[0].solveAttempts.length > num);
                    console.log('   got to puzzle ' + num + ': ' + (Math.round(numGotTo.length / totals * 1000) / 10) + '%', numGotTo.length)
                    if (num === 10) {
                        numGotTo.forEach(d => { 
                            num10 += 1;
                            num10Solved += d.experiments[0].solveAttempts[10].history.isSolved ? 1 : 0
                        })
                    }
                }
                console.log('  Number 10s:' + num10);
                console.log('  Number 10s Solved:' + (Math.round(num10Solved / num10 * 1000) / 10) + '%', num10Solved);
                
            }

            let finishedSecondQuestionaireAndNew = finishedSimpleTrainingExample.filter(d => d.questionaireStudy !== undefined);
            console.log('finishedSecondQuestionaire', finishedSecondQuestionaireAndNew.length)
            ...
            let drawAbleWindmillPuzzles = windmillPuzzles.filter(pMeta => pMeta.windmillData.width <= 4 && pMeta.windmillData.height <= 4 && (pMeta.windmillData.width > 1 || pMeta.windmillData.height > 1));

            function quickDraw(drawAbleWindmillPuzzles) {

                let puzzleMetaBlocks = d3.select('#updatesSection').selectAll('.puzzleMetaBlocks')
                ...
                puzzleMetaBlocksEntered
                    .each(function (windmillPuzzleMeta) {
                        ...
                        let puzzleSetup = convertWindmillPuzzleMetaToEugenePuzzleMeta(windmillPuzzleMeta);


                        if (windmillPuzzleMeta.totalInfoGainTrajectory2 === undefined) {
            ...
            quickDraw(drawAbleWindmillPuzzles); //.filter((d,i)=>i < 7)

...
function fake() {
    ...
    self.importScripts('../lib/WitnessPuzzle/WitnessPuzzle.js', '../lib/WitnessPuzzle/modules/_dependencies/d3/4.13.0/d3roll.min.js');
    ...
    if (showAll || showMultipleConstraintCombinations) {
        ...
        puzzleGrids.forEach(set => set.puzzles.forEach(d => {
            ...
            puzzleSetsByConstraints = generatePuzzlesWithConstraints(d, {
                numRegionConstraints: { exact: 9 },
                numColorOfRegionConstraints: { exact: 3 },
                numMustCrosses: { exact: 3 },
                // numCannotCrosses: { max: 1 }
            });

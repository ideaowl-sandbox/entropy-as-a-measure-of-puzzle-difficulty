...
setTimeout(function(){
    ...
    d3.queue()
        ...
        // .defer(d3.json, './data/windmill-2023-04-04-OUTPUT-from-Node-added-KL-min-soln-entropy.json')
        // .defer(d3.json, './data/windmill-2023-04-05-OUTPUT-from-Node-KL-entropy-with-pruning.json')
        // .defer(d3.json, './data/windmill-2023-04-05-OUTPUT-from-Node-TESTBED-EXAMPLES-KL-entropy-with-pruning.json')
        // .defer(d3.json, './data/windmill-2023-04-06-OUTPUT-from-Node-KL-entropy-with-pruning.json')
        // .defer(d3.json, './data/windmill-2023-04-10-OUTPUT-from-Node-TESTBED-EXAMPLES-KL-entropy-with-pruning-debug.json')
        // .defer(d3.json, './data/windmill-2023-04-11-OUTPUT-from-Node-KL-entropy-with-pruning-N0,1,2,5.json')
        .defer(d3.json, './data/OUTPUT-windmillTrianglePMetas.json')
    ...
    // 
        .awaitAll(function (err, [windmillPuzzles]) {
            ...
            drawAbleWindmillPuzzles = drawAbleWindmillPuzzles.map(pMeta => {
                pMeta.measures = [
                    { name: 'minSolKLDivergenceEntropyWithPruningAndN0', val: pMeta.ReMUSEn0 },
                    { name: 'minSolKLDivergenceEntropyWithPruningAndN2', val: pMeta.ReMUSEn2 },
                    { name: 'upvotes', val: pMeta.upvotes },
                    { name: 'solves', val: pMeta.solves },
                    { name: 'timeStampDate', val: pMeta.createUtc}
                ]

                pMeta.measureValsByName = {
                    minSolKLDivergenceEntropyWithPruningAndN0: pMeta.ReMUSEn0,
                    minSolKLDivergenceEntropyWithPruningAndN2: pMeta.ReMUSEn2,
                    upvotes: pMeta.upvotes,
                    solves: pMeta.solves,
                    timeStampDate: pMeta.createUtc
                }
                return pMeta
            })
            ...
            // drawAbleWindmillPuzzles = drawAbleWindmillPuzzles.filter(w => w.windmillData.height <= 4 && w.windmillData.height > 1)
            // drawAbleWindmillPuzzles = drawAbleWindmillPuzzles.filter(w => w.windmillData.width <= 4 && w.windmillData.width > 1)
            // debugger;
            ...
            if (filterWitnessPuzzles && !initialWindmillExtraction) {
                ...
                // drawAbleWindmillPuzzles = drawAbleWindmillPuzzles.filter(pMeta => pMeta.ignore!== true);
            ...
            function getStats(spreadName, puzzleSet) {
                ...
                let r = stats.correlationCoefficient('zz_m_' + spreadName, 'zz_m_' + 'minSolKLDivergenceEntropyWithPruningAndN0');
            ...
            if (!initialWindmillExtraction) {
                ...
                if (!puzzlesNOTfromWindmill) {
                    ...
                    for (let [spreadName, spreadObj] of Object.entries(measuresToSpread)) {
                        ...
                        // // Scores for upvotes
                        // if (spreadName === 'upvotes') {
                        //     spreadObj.scoresByType['byMinSolves'] = []
                        //     for (let i = 0; i < spreadObj.uniques.length; i++) {
                        //         let uniqueVal = solvesUniques[i];
                        //         let puzzleSet = drawAbleWindmillPuzzles.filter(puzMeta => puzMeta['solves'] >= uniqueVal);
                        //         spreadObj.scoresByType['byMinSolves'].push({
                        //             val: getStats(spreadName, puzzleSet),
                        //             minSolve: uniqueVal,
                        //             count: puzzleSet.length
                        //         });
                        //     }
            ...
            let filterPuzzlesSection = chartMiddleSection.append('div')
                .classed('filterPuzzlesSection', true)
            ...
            filterPuzzlesSection.append('div')
                .text('Toggles for puzzles to filter')
                .style('margin-bottom', '20px')
                .style('font-weight', 'bold')

            filterPuzzlesSection.append('div')
                .text('mustCross everywhere puzzles')
                .style('font-size', '11px')
                .style('cursor', 'pointer')
                .attr('data-toggled', '0')
                .on('click', function () {
                    let toggled = d3.select(this).attr('data-toggled');

                    let puzzlesFound = 0;
                    drawAbleWindmillPuzzles.forEach(pMeta => {
                        if (pMeta.puzzleSetup.constraints.mustCrosses !== undefined) {
                            let isSuchPuzzle = true;
                            const mcs = pMeta.puzzleSetup.constraints.mustCrosses.map(mc => JSON.stringify(mc));
                            const startPositionStr = JSON.stringify(pMeta.puzzleSetup.startPosition);
                            const endPositionStr = JSON.stringify([pMeta.puzzleSetup.endPosition[0], pMeta.puzzleSetup.endPosition[1]]);
                            for (let x = 0; x <= pMeta.puzzleSetup.size[0]; x++) {
                                for (let y = 0; y <= pMeta.puzzleSetup.size[1]; y++) {
                                    const posn = JSON.stringify([x, y]);
                                    if (posn !== endPositionStr && posn !== startPositionStr && !mcs.includes(posn)) {
                                        isSuchPuzzle = false;
                                        break;
                                    }
                                }
                            ...
                            if (isSuchPuzzle) {
                                pMeta.ignore = toggled === '0' ? true : false;
                                puzzlesFound++;
                                d3.select(this).attr('data-toggled', toggled === '0' ? '1' : '0');
                            }
                        }
                        
                    })
                    console.log({puzzlesFound})
                    chartDraw(drawAbleWindmillPuzzles);
                })
            ...
            function chartDraw(selectWindmillPuzzles) {
                ...
                function updateCorrelationScoreWithFiltering() {
                    ...
                    if (linearResults !== undefined) {
                        ...
                        r = stats.correlationCoefficient('zz_m_upvotes', 'zz_m_minSolKLDivergenceEntropyWithPruningAndN0');
                    ...
                    .each(function (windmillPuzzleMeta) {
                        ...
                        if (!initialWindmillExtraction) {
                            ...
                            metaInfo.append('div')
                                ...
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolEntropy2N0: ' + (+windmillPuzzleMeta.measureValsByName.minSolutionEntropy2N0).toPrecision(3))
                                .on('click', function () {
                            ...
                            metaInfo.append('div')
                                ...
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolEntropy2N1: ' + (+windmillPuzzleMeta.measureValsByName.minSolutionEntropy2N1).toPrecision(3))
                                .on('click', function () {
                            ...
                            metaInfo.append('div')
                                ...
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolEntropy2N2: ' + (+windmillPuzzleMeta.measureValsByName.minSolutionEntropy2N2).toPrecision(3))
                                .on('click', function () {
                            ...
                            metaInfo.append('div')
                                .classed('isMeta', true)
                                .classed('minSolEntropy2N2wExits', true)
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolEntropy2N2wExits: ' + (+windmillPuzzleMeta.measureValsByName.minSolutionEntropy2N2wStraightExits).toPrecision(3))
                                .on('click', function () {
                                    quickDraw(drawAbleWindmillPuzzles.sort((a, b) => (+b.minSolutionEntropy2N2wStraightExits) - (+a.measureValsByName.minSolutionEntropy2N2wStraightExits)))
                                    d3.selectAll('.minSolEntropy2N2wExits').classed('isSorted', true);
                                })
                            ...
                            metaInfo.append('div')
                                ...
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolKLEntropyPruneN0: ' + (+windmillPuzzleMeta.measureValsByName.minSolKLDivergenceEntropyWithPruningAndN0).toPrecision(3))
                                .on('click', function () {
                            ...
                            metaInfo.append('div')
                                ...
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolKLEntropyPruneN1: ' + (+windmillPuzzleMeta.measureValsByName.minSolKLDivergenceEntropyWithPruningAndN1).toPrecision(3))
                                .on('click', function () {
                            ...
                            metaInfo.append('div')
                                ...
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolKLEntropyPruneN2: ' + (+windmillPuzzleMeta.measureValsByName.minSolKLDivergenceEntropyWithPruningAndN2).toPrecision(3))
                                .on('click', function () {
                            ...
                            metaInfo.append('div')
                                .classed('isMeta', true)
                                .classed('minSolKLEntropyPruneN5', true)
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolKLEntropyPruneN5: ' + (+windmillPuzzleMeta.measureValsByName.minSolKLDivergenceEntropyWithPruningAndN5).toPrecision(3))
                                .on('click', function () {
                                    quickDraw(drawAbleWindmillPuzzles.sort((a, b) => (+b.minSolKLDivergenceEntropyWithPruningAndN5) - (+a.measureValsByName.minSolKLDivergenceEntropyWithPruningAndN5)))
                                    d3.selectAll('.minSolKLEntropyPruneN5').classed('isSorted', true);
                                })


                            metaInfo.append('div')
                                .classed('isMeta', true)
                                .classed('minSolKLEntropy', true)
                                .style('display', 'inline-block')
                                .style('margin-right', '10px')
                                .text('minSolKLEntropy: ' + (+windmillPuzzleMeta.measureValsByName.minSolKLDivergenceEntropy).toPrecision(3))
                                .on('click', function () {
                                    quickDraw(drawAbleWindmillPuzzles.sort((a, b) => (+b.minSolKLDivergenceEntropy) - (+a.measureValsByName.minSolKLDivergenceEntropy)))
                                    d3.selectAll('.minSolKLEntropy').classed('isSorted', true);
                                })
